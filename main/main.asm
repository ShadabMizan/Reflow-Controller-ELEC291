;===================================================================
; CV-8052 Temperature Measurement with Serial Echo
; Combines temperature monitoring with serial command reception
; - Continuously reads and displays temperature from thermocouple
; - Non-blocking serial receive - echoes any received characters
;===================================================================
...
y:                  ds 4
bcd:                ds 5
uart_buffer:        ds 10  ; Buffer to store incoming characters
uart_value:         ds 4   ; Final decimal value (32-bit)
uart_index:         ds 1   ; Current position in buffer
...
;   - Echo it back
;   - If it's a digit (0-9), add to buffer
;   - If it's Enter/CR/LF, convert buffer to decimal and store
;-------------------------------------------------------------------
Check_And_Echo_Serial:
	jnb RI, Check_Serial_Done  ; Check if receive flag is set
	clr RI                      ; Clear receive flag
	mov a, SBUF                 ; Get received character
	lcall putchar               ; Echo it back
	
	; Check if it's a carriage return (0x0D) or line feed (0x0A)
	cjne a, #0x0D, Check_LF
	ljmp Convert_Buffer_To_Decimal
Check_LF:
	cjne a, #0x0A, Check_Digit
	ljmp Convert_Buffer_To_Decimal
...
Check_Digit:
	...
	clr c
	subb a, #'0'
	jc Check_Serial_Done    ; Less than '0', ignore
	mov b, a                ; Save (char - '0')
	clr c
	subb a, #10
	jnc Check_Serial_Done   ; Greater than '9', ignore
	
	; Valid digit - add to buffer
	mov a, uart_index
	mov r0, a
	add a, #uart_buffer
	mov r1, a
	mov a, b                ; Restore digit value (0-9)
	mov @r1, a              ; Store in buffer
	inc uart_index
	; Prevent buffer overflow
	mov a, uart_index
	cjne a, #9, Check_Serial_Done
	mov uart_index, #9      ; Limit to 9 digits

Check_Serial_Done:
	ret

;-------------------------------------------------------------------
; Convert ASCII buffer to decimal value
; Converts the digits in uart_buffer to a 32-bit value in uart_value
; Example: buffer contains [1,0,0] → uart_value = 100
;-------------------------------------------------------------------
Convert_Buffer_To_Decimal:
	; Check if buffer is empty
	mov a, uart_index
	jz Clear_Buffer_Exit
	
	; Initialize uart_value to 0
	mov uart_value+0, #0
	mov uart_value+1, #0
	mov uart_value+2, #0
	mov uart_value+3, #0
	...
	mov r0, #uart_buffer
	mov r2, uart_index      ; Number of digits to process
	
Convert_Loop:
	; Load current uart_value into x
	mov x+0, uart_value+0
	mov x+1, uart_value+1
	mov x+2, uart_value+2
	...
	; Store result back to uart_value
	mov uart_value+0, x+0
	mov uart_value+1, x+1
	mov uart_value+2, x+2
	mov uart_value+3, x+3
	
	; Next digit
	inc r0
	djnz r2, Convert_Loop
	
	; Clear buffer for next input
	mov uart_index, #0
	
	; Optional: Send confirmation (uncomment if desired)
	; mov a, #'O'
	; lcall putchar
	; mov a, #'K'
	; lcall putchar
...
mycode:
	...
	; Main loop: continuously read temperature and check for serial input
main_loop:
	; Check for any incoming serial characters and echo them
	lcall Check_And_Echo_Serial
	
	; Update reference voltage
	lcall LM4040_ADC
	
	; Check for serial input again
	lcall Check_And_Echo_Serial
	...
	; Read thermocouple and display temperature
