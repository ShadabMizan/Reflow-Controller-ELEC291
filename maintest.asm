...
PARAM_PIN       EQU P2.3    ; 0=Temp, 1=Time
MODE_PIN        EQU P2.4    ; 0=Soak, 1=Reflow
; P2.5 is IGNORE
; 8-bit value: P2.6 (bit7/MSB) P2.7 (bit6) P3.0 (bit5) P3.1 (bit4) P3.2 (bit3) P3.3 (bit2) P3.4 (bit1) P3.5 (bit0/LSB)
...
forever:
    ...
    ; Read parameters only when in state 0 (before FSM starts)
    mov a, fsm_state
    jnz skip_param_read
    lcall Read_Parameters
skip_param_read:
...
Read_Parameters:
    ...
    ; Read the 8-bit value from pins (big-endian)
    ; Bit 7 (MSB): P2.6
    ; Bit 6: P2.7
    ; Bit 5: P3.0
    ; Bit 4: P3.1
    ...
    ; Now A contains the 8-bit value (0-255)
    ; Determine which parameter to update based on PARAM_PIN and MODE_PIN
    
    jb PARAM_PIN, Param_Is_Time
    ; PARAM_PIN = 0, so it's Temperature
    jb MODE_PIN, Param_Is_Reflow_Temp
    ; MODE_PIN = 0, so it's Soak Temperature
    mov soaktmp, A
    sjmp Read_Params_Done
    
Param_Is_Reflow_Temp:
    ; MODE_PIN = 1, so it's Reflow Temperature
    mov reflowtmp, A
    sjmp Read_Params_Done
    
Param_Is_Time:
    ; PARAM_PIN = 1, so it's Time
    jb MODE_PIN, Param_Is_Reflow_Time
    ; MODE_PIN = 0, so it's Soak Time
    mov soaktime, A
    sjmp Read_Params_Done
    
Param_Is_Reflow_Time:
    ; MODE_PIN = 1, so it's Reflow Time
    mov reflowtime, A
